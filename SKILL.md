# yolo-sprint -- overnight usage-surplus speculative research run
...
Run: `& "<WORKSPACE>\yolo-sprint\usage_gate.ps1"`
...
## Step 0.5 -- CLOSE OUT prior sprints (~2 min; only reached on a RUN verdict)
If you created triage items for prior sprints (see Step 4), sweep them: an item
...
like this: prefer a project not hit by a recent sprint (check
`<WORKSPACE>\yolo_sprints\` for prior run folders), skip meta/infrastructure
...
tonight's sprint folder DURING this step, not after -- domain, each source URL,
...
throwaway sketch in `<WORKSPACE>\yolo_sprints\<YYYY-MM-DD>_<project>\` (create the
folder; everything disposable lives there).
...
Write `brief.md` in tonight's sprint folder: project + domain, the 2-3 pairings
...
`<WORKSPACE>\yolo_sprints\ledger.jsonl` -- date, project, domain, folder,
...
rather than editing history. This is what makes a month of sprints ANALYZABLE:
...
(Obsidian base, kanban file, etc.), also create ONE item there per sprint
...
state files properly -- the sprint deliberately skimmed), then execute inside that
